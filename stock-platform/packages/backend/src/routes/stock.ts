...
export async function stockRoutes(app: FastifyInstance) {
  ...
  // Get a single lot stock detail — delegates core computation to StockService
  app.get<{ Params: { lotId: string } }>('/:lotId', async (request, reply) => {
    const isAuthenticated = !!(request as any).user;
    const lotStock = await stockService.getLotStock(request.params.lotId);

    if (!lotStock) return reply.notFound('Stock lot not found');

    // Fetch sale history separately (StockService doesn't return individual sale lines)
    ...
    return {
      ...
      refNumber: lotStock.refNumber,
      date: lotStock.date,
      category: lotStock.category,
      designation: lotStock.designation,
      supplier: lotStock.supplier,
      boutique: lotStock.boutique,
      initialQuantity: lotStock.initialQuantity,
      soldQuantity: lotStock.soldQuantity,
      remainingQuantity: lotStock.remainingQuantity,
      targetResalePrice: lotStock.targetResalePrice,
      ...(isAuthenticated && {
        purchaseUnitCost: lotStock.purchaseUnitCost,
        currentStockValue: lotStock.currentStockValue,
      }),
      saleHistory: lot?.saleLines ?? [],
    };
  });

  // Summary: total stock value grouped by category — delegates to StockService
  app.get('/summary/by-category', async (request) => {
    const isAuthenticated = !!(request as any).user;
    const { summary: fullSummary, grandTotal } = await stockService.computeSummaryByCategory();

    const summary = fullSummary.map(r => ({
      category: r.category,
      ...(isAuthenticated && { totalValue: r.totalValue }),
      totalLots: r.totalLots,
      totalRemaining: r.totalRemaining,
    }));

    return { summary, ...(isAuthenticated && { grandTotal }) };
  });
