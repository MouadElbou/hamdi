...
export function BatteryRepairPage(): React.JSX.Element {
  const { batteryTariffs: tariffs, refreshTariffs } = useReferenceData();
  ...
  const [newTariffForm, setNewTariffForm] = useState({ label: '', particuliers: '', rev: '' });
  ...
  const load = () => {
    window.api.batteryRepair.list({ search: debouncedSearch || undefined, page, limit: PAGE_SIZE }).then((r: unknown) => {
      const data = r as { items: Job[]; total: number };
      setJobs(data.items || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await window.api.batteryRepair.update({ id: editingId, ...payload });
    } else {
      await window.api.batteryRepair.create(payload);
    }
    closeForm();
    load();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(true);
    try {
      await window.api.batteryRepair.delete(id);
      load();
    } catch (err) {
      ...
      {tariffs.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div className="card-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            Grille tarifaire
            {!addingTariff && <button className="btn btn-secondary btn-sm" onClick={() => { setNewTariffForm({ label: '', particuliers: '', rev: '' }); setAddingTariff(true); }}>+ Ajouter</button>}
          </div>
          <div className="table-wrapper">
            <table className="data-table">
              ...
              <tbody>
                      ...
                      <td>
                        <div className="row-actions">
                          <button className="btn-icon" title="Sauvegarder" onClick={async () => {
                            try {
                              await window.api.batteryRepair.updateTariff({ id: t.id, label: tariffForm.label, particuliersPrice: tariffForm.particuliers !== '' ? parseCents(tariffForm.particuliers, 'Prix particuliers') : null, revPrice: tariffForm.rev !== '' ? parseCents(tariffForm.rev, 'Prix revendeur') : null });
                              setEditingTariffId(null); refreshTariffs();
                    ...
                    <tr key={t.id}>
                      ...
                      <td>
                        <div className="row-actions">
                          ...
                          <button className="btn-icon btn-icon-danger" title="Supprimer" onClick={async () => {
                            if (!await confirm('Supprimer ce tarif ?')) return;
                            try {
                              await window.api.batteryRepair.deleteTariff(t.id);
                              refreshTariffs();
                ...
                {addingTariff && (
                  <tr>
                    <td><input type="text" value={newTariffForm.label} onChange={e => setNewTariffForm({ ...newTariffForm, label: e.target.value })} placeholder="Nom du service" style={{ width: '100%' }} autoFocus /></td>
                    <td><input type="number" step="0.01" value={newTariffForm.particuliers} onChange={e => setNewTariffForm({ ...newTariffForm, particuliers: e.target.value })} placeholder="Sur devis" style={{ width: 100, textAlign: 'right' }} /></td>
                    <td><input type="number" step="0.01" value={newTariffForm.rev} onChange={e => setNewTariffForm({ ...newTariffForm, rev: e.target.value })} placeholder="Sur devis" style={{ width: 100, textAlign: 'right' }} /></td>
                    <td>
                      <div className="row-actions">
                        <button className="btn-icon" title="Ajouter" onClick={async () => {
                          if (!newTariffForm.label.trim()) return;
                          try {
                            await window.api.batteryRepair.createTariff({ label: newTariffForm.label.trim(), particuliersPrice: newTariffForm.particuliers !== '' ? parseCents(newTariffForm.particuliers, 'Prix particuliers') : null, revPrice: newTariffForm.rev !== '' ? parseCents(newTariffForm.rev, 'Prix revendeur') : null });
                            setAddingTariff(false); refreshTariffs();
                          } catch (err) { addToast((err as Error).message || 'Erreur lors de la creation du tarif', 'error'); }
                        }}>✓</button>
                        <button className="btn-icon" title="Annuler" onClick={() => setAddingTariff(false)}>{CancelIcon}</button>
                      </div>
                    </td>
                  </tr>
                )}
      ...
      {tariffs.length === 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          {addingTariff ? (
            ...
            <div className="empty-state" style={{ padding: '16px' }}><div className="empty-desc">Aucun tarif. Cliquez sur "+ Ajouter" pour commencer.</div></div>
          )}
        </div>
      )}
