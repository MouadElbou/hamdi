...
export function AdminUsersPage(): React.JSX.Element {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [u, e] = await Promise.all([
        window.api.users.list() as Promise<UserRecord[]>,
        window.api.employees.list() as Promise<Employee[]>,
      ]);
      setUsers(u);
      setEmployees(e);
    } catch (err: unknown) {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await window.api.users.resetPassword({ userId: showResetModal, newPassword: resetPwd });
